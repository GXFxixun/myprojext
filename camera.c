...
u8 rgbbuf[320 * 240 * 3];
/*用于存储bmp图片*/
u8 bmpbuf[54 + 320 * 240 * 3];
/*用于存储jpg图片*/
extern u8 *jpgbuf;
extern unsigned long int size_jpg;
...
int set_cap_fmt(int fd)
{
	...
	capfmt.fmt.pix.width = 320;
	capfmt.fmt.pix.height = 240;
	capfmt.fmt.pix.pixelformat = V4L2_PIX_FMT_YUYV;
	capfmt.fmt.pix.field = V4L2_FIELD_ANY;
...
void process_image(const void *p)
{
	...
	char *pbuf = (char *)p;
	memcpy(yuvbuf,pbuf,320*240*2);
	init_conv(320,240);
	yuvtorgb24(rgbbuf, yuvbuf, 320, 240);
	getbmp(bmpbuf,320,240);
	pthread_mutex_lock(&mutex);
	size_jpg = rgbtojpeg(&jpgbuf,rgbbuf, 320, 240);
	uninit_conv();
	/*	
	int file = -1;
	file = open("1.bmp", O_RDWR | O_CREAT, 0666);
	if(file > 0)
	{
		write(file,bmpbuf,320*240*3+54);
		close(file);
		//exit(0);	
	}
	*/	
